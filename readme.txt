...
BP Labs contains three unofficial BuddyPress experiments; *@mentions autosuggest*, *Quick Admin*, and *Like Button*. All experiments are in beta, and come with no guarantees.

*@mentions autosuggest* requires the Activity component, and extends its @messaging feature to help you find the short name of a user. It is integrated into comments, the "What's New" activity status box, Private Messaging (body), and bbPress v1 group forums. To trigger the autosuggest, type an `@` followed by at least one other letter.
...
*Like Button* requires the Activity component, and adds a "Like" button onto WordPress' Toolbar when viewing a blog post.
...
* Add Like Button experiment (requires BuddyPress 1.6+ & WordPress 3.4+).
* Thanks to karmatosed, and Ryan McCue, for graphics and bug fixes, respectively.
